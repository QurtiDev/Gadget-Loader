...
This program is a Windows shellcode loader written in Go. What it does is download a shellcode blob from provided URL, injects it into the current process' memory and executes it. It benefits from the usage of [BananPhone](https://github.com/C-Sto/BananaPhone) to make indirect system calls instead of calling the Windows API directly, which helps support our goal of avoiding detection by EDR/XDR solutions! 
